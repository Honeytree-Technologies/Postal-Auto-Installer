...
########################################################################
#                                                                      #
#               Postal Installation and Hardening Script               #
#                                                                      #
#                  Created by Honeytree Technologies, LLC              #
#                            www.honeytreetech.com                     #
#                                                                      #
#                      Postal: honeytree.social                        #
...
#####                                                              #####
#####   This will install Postal on fresh server                   #####
#####                                                              #####
#####  Installing on an operating Postal server will wipe data     #####
#####                                                              #####
########################################################################
startup_warning
...
validate_port() {
    local port=$1
    local excluded_ports=("80" "443" "3306" "5672" "5000")
...
while true; do
  ...
  if [ -n "$smtp_password" ]; then
    ...
    echo "SMTP password cannot be empty. Please enter SMTP password."
  fi
done

while true; do
  read -p "Enter SMTP from name: " smtp_from_name
  if [ -n "$smtp_from_name" ]; then
    break
  else
    echo "SMTP from name cannot be empty. Please enter SMTP from name."
  fi
done
...
temp_password="pass_$(random_string 8)"
read -p "Enter the  DB ROOT PASSWORD (Default: ${temp_password}): " mariadb_root_password
if [ -z ${mariadb_root_password} ] ; then
  mariadb_root_password=${temp_password}
fi
echo "your DB root password is ${mariadb_root_password}"




temp_password="pass_$(random_string 8)"
read -p "Enter the rabbitmq PASSWORD (Default: ${temp_password}): " rabbitmq_password
if [ -z ${rabbitmq_password} ] ; then
  rabbitmq_password=${temp_password}
fi
...
while true; do
  read -p "Enter a port number (1-65535, excluding 80, 443, 3306, 5672, 5000): " port
  # Validate the input
  validate_port "$port"
  case $? in
    0)
...
#  Remove Old applciaiton data if exist
sudo rm -rf /opt/postal/
sudo rm -f /usr/bin/postal
sudo rm ~/docker-compose.yml
sudo rm -rf /postal-application-data


# RRun the docker container for the mariadb and rabbitmq
docker run -d \
 --name postal-mariadb \
 ...
 -v /postal-application-data/mariadb:/var/lib/mysql \
 mariadb
 ...
 -v /postal-application-data/rabbitmq/data/:/var/lib/rabbitmq/ \
 -v /postal-application-data/rabbitmq/log/:/var/log/rabbitmq \
 rabbitmq:3.8

 sleep 15


# Git clone of postal application in /opt/postal/install directory
git clone https://postalserver.io/start/install /opt/postal/install
sudo ln -s /opt/postal/install/bin/postal /usr/bin/postal


# Initialize the domain for application file
postal bootstrap ${domain_name}


cat <<postal_yml >> /opt/postal/config/postal.yml
general:
...
dns:
  ...
  mx_records:
    - mx.${domain_name}
  smtp_server_hostname: ${domain_name}
  spf_include: spf.${domain_name}
  return_path: rp.${domain_name}
  route_domain: routes.${domain_name}
  track_domain: track.${domain_name}
...
smtp:
  ...
  host: ${smtp_host}
  port: ${smtp_port}
  username:  ${smtp_username}# Complete when Postal is running and you can
  password:  ${smtp_password}#  generate the credentials within the interface.
...
# Run predefined postal commands
postal initialize
postal make-user
postal start
postal status
...
if nginx -v &>/dev/null; then
 rm -f /etc/nginx/conf.d/${domain_name}.conf
else
...
#  Generte the SSL certificate for the domain
apt-get install certbot -y
certbot certonly --standalone -d ${domain_name}

cat <<nginx_file >> /etc/nginx/conf.d/${domain_name}.conf
server {
        listen [::]:80;
        listen 0.0.0.0:80;
